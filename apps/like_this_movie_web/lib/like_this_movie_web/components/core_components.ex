defmodule LikeThisMovieWeb.CoreComponents do
  @moduledoc """
          ...
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            ...
            >
              <div class="absolute top-6 right-5">
                ...
                >
                  <.icon name="material-close" class="h-5 w-5" />
                </button>
              </div>
              <div id={"#{@id}-content"}>
  ...
  def flash(assigns) do
    ...
    >
      ...
      <button type="button" class="group absolute top-1 right-1 p-2" aria-label={gettext("close")}>
        <.icon name="material-close" class="h-5 w-5 opacity-40 group-hover:opacity-70" />
      </button>
    </div>
    """
  end
  ...
  def error(assigns) do
    ...
    <p class="mt-3 flex gap-3 text-sm leading-6 text-rose-600 phx-no-feedback:hidden">
      <.icon name="material-error" class="mt-0.5 h-5 w-5 flex-none" />
      <%= render_slot(@inner_block) %>
    </p>
    """
  end
  ...
  def back(assigns) do
    ...
    <div class="mt-16">
      ...
      >
        <.icon name="material-arrow_back" class="h-3 w-3" />
        <%= render_slot(@inner_block) %>
      </.link>
    </div>
    """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(LikeThisMovieWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(LikeThisMovieWeb.Gettext, "errors", msg, opts)
    end
  end
